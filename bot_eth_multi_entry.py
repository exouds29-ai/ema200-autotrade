# bot_eth_multi_entry.py
# Bybit USDT Perp (ETH/USDT:USDT) — One-way(단방향) 전용
...
# - 조건은 솔라나 완화판과 동일, ETH에 맞는 틱/수량만 다름
...
TP1_RATIO      = 0.50
BREAKEVEN_TICK = 0.10                   # ETH는 0.1 정도 여유
...
def fetch_ohlcv(tf, limit=300):
    ...
    # 정수 인덱스(0..N-1) 유지 — 채널 계산 시 좌표로 사용
    return df
...
def swing_points(series, left=3, right=3, kind='low'):
    ...
    for i in range(left, len(s)-right):
        ...
        if kind=='low'  and s.iloc[i] == win.min(): idxs.append(i)  # 정수 인덱스
...
def run_once():
    ...
    m15 = build_frame(TF_15M, 300)  # 현재는 가점용 모듈과 분리(필요 시 확장)
    ...
    has_pos = in_position()
    print(f"[HB] {now_utc()} ETH px={last:.2f} trend(4H)={trend} conf={score} inPos={has_pos}")
